...
#                       Prefix="DES",
#                       Control="XTWT-S11,XTWT-S11,XTWT-S25,TELS-S11,",
#                       Treat="TELS-S11,TelsDp53-S11,TelsDp53-S25,TelsDp53-S11",
#                       Condition="f:/Project/SZY/hybridize/RNA/scripts/sample.info.txt",
#                       GeneLens="f:/Project/Xenopus/Xenopus.gene.len.txt",
#                       Species="hsa", ## xtr xla hsa dre
#                       Include=NULL,
#                       GO=T, KEGG=T, TPM=F, MaxP=0.01, MinLog2FC=1) #测试参数
## options----------------------------------------------------------------------------------------------------------------------------------------------------
# 从命令行接收参数
...
###################测试代码###################
# gene_index = c(1:length(entrez_id))
# geneList = res_dataframe$log2FoldChange[gene_index]
# names(geneList) = entrez_id[gene_index]
# geneList = sort(geneList,decreasing = T)
# gseaKEGG <- gseKEGG(geneList = geneList,organism = 'hsa',pvalueCutoff = 1,verbose = FALSE)
# gseaGO <- gseGO(geneList,ont="all",OrgDb = data_base,pvalueCutoff = 1,verbose = F)
####################################################################################
...
for (i in 1:length(TreatList)) {
  ...
  volcano_fig_list[[title_name]] <- ggplot(data = res_dataframe) + theme_classic() + 
    ...
    ggtitle(title_name) + theme(plot.title = element_text(hjust = 0.5), plot.margin = unit(rep(1,4),'lines'),legend.title = element_blank())+
    theme(title  = element_text(colour = "black",size = 6), 
          axis.text  = element_text(colour = "black",size = 5), 
          axis.line  = element_line(size = 0.5,color = "black"),legend.position = "top") # 绘制火山图 
  ...
  if (length(gene_index) > 0) {
    ...
    if (!is.null(data_base) && !is.null(opt$GO) && opt$GO) {
      ...
      if (!is.null(go_result$ALL) && dim(go_result$ALL@result)[1] > 0) {
        GO_fig_list[[paste(aSample, " vs ", aControl, sep = "")]] <- dotplot(go_result$ALL, title = paste(aSample, " vs ", aControl, " Diff genes\tGO term", sep = ""), 
      ...
      if (!is.null(go_result$ALL) && dim(go_result$ALL@result)[1] > 0) {
        Up_GO_fig_list[[paste(aSample, " vs ", aControl, sep = "")]] <- dotplot(go_result$ALL, title = paste(aSample, " vs ", aControl, " Up genes\tGO term", sep = ""), 
                                                                                showCategory = 5, split = "ONTOLOGY")+facet_grid(ONTOLOGY~., scale="free")
      ...
      if (!is.null(go_result$ALL) && dim(go_result$ALL@result)[1] > 0) {
        Down_GO_fig_list[[paste(aSample, " vs ", aControl, sep = "")]] <- dotplot(go_result$ALL, title = paste(aSample, " vs ", aControl, " Down genes\tGO term", sep = ""), 
                                                                                  showCategory = 5, split = "ONTOLOGY")+facet_grid(ONTOLOGY~., scale="free")
    ...
    if (!is.null(opt$KEGG) && !is.null(Species) && opt$KEGG) {
      ...
      # pathway_data <- res_dataframe$log2FoldChange
      # names(pathway_data) <- entrez_id
      ###############Diff gene KEGG#####################
      ...
      if(!is.null(kegg_result$term) && dim(kegg_result$term )[1] > 0){
        ...
        ##绘制kegg通路图，之绘制1前5个通路
        # pv.out <- pathview::pathview(gene.data = pathway_data[gene_index], species = Species, pathway.id = pathway_ids[1:min(5,length(pathway_ids))], kegg.dir = OutDir, )
      ###############Up gene KEGG######################
...
plot_grid(volcano_fig_list[[1]],volcano_fig_list[[2]],volcano_fig_list[[3]],volcano_fig_list[[4]],nrow = 1)
test<-warnings()
#--------------------------------------------------------------------------------------
